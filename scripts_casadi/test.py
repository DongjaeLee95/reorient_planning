...
u = vertcat(alpha,beta,eps1,eps2)
...
ocp.constraints.lbu = np.array([phidot_lb_, phidot_lb_, 0, 0])
ocp.constraints.ubu = np.array([phidot_ub_,phidot_ub_,math.inf, math.inf])
ocp.constraints.idxbu = np.array([0,1,2,3])
ocp.constraints.C_e = np.array([[0.0, 1.0],[(y1-y2)/x1, -1.0], [-(y1-y2)/x1, -1.0]])
ocp.constraints.lg_e = np.array([[y1], [-y2], [-y2]]).flatten()
ocp.constraints.ug_e = np.array([[math.inf], [math.inf], [math.inf]]).flatten()
ocp.constraints.lh_e = umin_.flatten()
ocp.constraints.uh_e = umax_.flatten()

ocp.constraints.x0 = np.array([0.0, 0.0]) # plug in previous value!
...
ocp.solver_options.integrator_type = 'ERK' # ERK (Explicit Runge-Kutta), IRK, GNSF, DISCRETE
